...
const ColumnHeader = ({ column, title = "Header", child = false }) => {
  ...
  if (!canHide && !canPin && !canSort) return <span>{title}</span>

  const handleSorting = desc => column.toggleSorting(desc, true)

  const handleHide = () => !child && column.toggleVisibility(false)

  const handlePin = () => !child && column.pin(isPinned ? false : "left")

  const handleReset = () => isSorted && column.clearSorting()
