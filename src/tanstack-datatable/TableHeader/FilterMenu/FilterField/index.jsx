...
const generateFilter = ({ filteredColumn, setColumnFilters }) => {
  const type = filteredColumn.columnDef?.meta?.filter?.type

  switch (type) {
    case "multiselect":
      return <Multiselect column={filteredColumn} setColumnFilters={setColumnFilters} />

    case "daterange":
      return <Daterange column={filteredColumn} setColumnFilters={setColumnFilters} />

    case "timerange":
      return <Timerange column={filteredColumn} setColumnFilters={setColumnFilters} />

    case "range":
      return <Range column={filteredColumn} setColumnFilters={setColumnFilters} />
...
const ColumnMenu = ({ columns = [], filteredColumn, setColumnFilters }) => {
  ...
  const handleMenuItemClick = column => {
    setColumnFilters(prev => prev.map(col => (col.id === filteredColumn.id ? { id: column.id, value: undefined } : col)))
  }
...
const FilterField = ({ filter, filteredColumn, columns, setColumnFilters }) => {
  const onFilterRemove = id => setColumnFilters(prev => prev.filter(column => column.id !== id))
  ...
  return (
    <div className={styles["ts-dt-filter-menu-filter-field"]}>
      ...
      {generateFilter({ filteredColumn, setColumnFilters })}
