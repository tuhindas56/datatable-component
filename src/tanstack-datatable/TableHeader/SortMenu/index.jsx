...
const SortMenu = ({ table, direction = "left", enableMultiSort }) => {
  ...
  const onSortAdd = () => {
    if (!enableMultiSort && sorting.length === 1) return
  ...
  return (
    <>
      <Button onClick={handleClick} ref={anchorRef}>
        <SwapVertIcon /> Sort{" "}
        {sorting && sorting.length > 0 && <Chip label={sorting && sorting.length} sx={{ marginLeft: 0.6 }} />}
      </Button>
      <Popover
      ...
      >
        <Stack gap={2} sx={{ minWidth: 380 }} className="sort-popover-container bottom-anchored">
          ...
          <Stack direction="row" gap={1.4}>
            <Button
              className="ts-dt-add-sort-filter-button"
              onClick={onSortAdd}
              disabled={enableMultiSort && columns.length === 0 || !enableMultiSort && sorting.length > 0}
            >
              Add sort
            </Button>
            {sorting && sorting.length !== 0 && <Button onClick={onSortReset}>Reset sorting</Button>}
          </Stack>
        </Stack>
      </Popover>
    </>
